...
class SpeciesGMM:
    ...
    def fit(self, fme_khz: np.ndarray) -> "SpeciesGMM":
        x_1d = np.asarray(fme_khz)
        x = x_1d.reshape(-1, 1)
        kde = stats.gaussian_kde(x_1d, bw_method=self.bandwidth_method)
        self.kde_grid_ = np.linspace(x_1d.min(), x_1d.max(), self.grid_size)
        ...
        self.model = GaussianMixture(
            n_components=int(self.kde_peaks_.size),
            n_init=self.n_init,
    ...
    def params(self) -> dict[str, int | str | np.ndarray]:
        ...
        return {
            ...
            "K_detection_method": "kde_meanshift",
